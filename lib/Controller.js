...
const Controller = {
    ...
    init(pollIntervall, displayAddress, sensorPIN, sensorType, goodTempMin, goodTempMax, goodHumMin, goodHumMax, mediumDelta, readCallback) {
    ...
    read() {
        ...
        if( ( (data.temperature <= this.goodTempMax ) && (data.temperature >= this.goodTempMin) ) &&
            ...
            ( data.temperature >= ( data.temperature * (data.temperature * this.mediumDelta) / 100 ) ||
            data.temperature <= ( data.temperature * (data.temperature * this.mediumDelta) / 100 ) ) ||
            ( data.humidity >= ( data.humidity * (data.humidity * this.mediumDelta) / 100 ) ||
                data.humidity <= ( data.humidity * (data.humidity * this.mediumDelta) / 100 ) )
        ) {
            this.greenLED.off();
            this.yellowLED.on();
